---
title: "Semana 3: Comparación entre grupos"
format: revealjs
execute:
  echo: true
  warning: false
...
La función `t.test()` tambien puede utilizarse para compara dos medias, con una ligera modificacion en el codigo, debemos especificar nuestra variable respuesta y explicativa utilizando `~`, se puede leer como la variable ocio esta explicada (`~`) a travez de la variable genero.
...
Esto significa que aceptaremos hasta un 5% de probabilidad de cometer un error al rechazar $H_0$ si en realidad es verdadera.
...
f_plot = tibble(
  x = x_f,
  y = df(x_f, df1 = 4, df2 = 25)
)

ggplot(f_plot, aes(x = x, y = y)) +
  geom_line(color = "#38BDF8", linewidth = 1.2) +
  geom_area(fill = "#38BDF8", alpha = 0.18) +
  labs(
    title = "Distribución F",
    subtitle = "Ejemplo con gl1 = 4 y gl2 = 25",
    x = "Valor F",
    y = "Densidad"
  ) +
  theme_minimal()
...
Para verificar si la variabilidad detectada en el ANOVA se debe principalmente a **manufactura**, realizamos nuevamente la prueba excluyendo este sector y comparando únicamente **Tecnología, Finanzas, Educación y Salud**.
...
#| fig-width: 18
#| fig-height: 10
x_chi = seq(0, 15, length.out = 1000)
chi_plot = tibble(
  x = x_chi,
  y = dchisq(x_chi, df = 2)
)

ggplot(chi_plot, aes(x = x, y = y)) +
  geom_line(color = "#A78BFA", linewidth = 1.2) +
  geom_area(fill = "#A78BFA", alpha = 0.18) +
  labs(
    title = "Distribución Chi-cuadrado",
    subtitle = "Ejemplo con 2 grados de libertad",
    x = expression(chi^2),
    y = "Densidad"
  ) +
  theme_minimal()
