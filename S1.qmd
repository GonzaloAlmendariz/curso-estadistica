...
-   **Edad:** [variable numérica (típicamente tratada como continua, aunque se registre en enteros).]{.fragment}

-   **Sexo:** [variable categórica nominal.]{.fragment}

-   **Región:** [variable categórica nominal.]{.fragment}

-   **Nivel educativo:** [variable categórica ordinal (existe un orden natural: secundaria \< técnica \< universitaria).]{.fragment}

-   **Situación laboral:** [variable categórica nominal (ocupado, desocupado, estudiante, etc.).]{.fragment}
...
class(encuesta$edad)
class(encuesta$sexo)
class(encuesta$region)
class(encuesta$educacion)
```

------------------------------------------------------------------------

### **Ejercicio 3. Exploración inicial de los datos**
...
```{r}
#| echo: true
dlookr::diagnose_numeric(encuesta)
```
...
#| echo: true
#| eval: false
dlookr::describe(encuesta)
```
::: {style="max-width:100%; overflow-x:auto; white-space:pre;"}
```{r}
#| results: asis
library(dlookr)
library(knitr)

tab <- dlookr::describe(encuesta) |> as.data.frame()

cat(knitr::kable(tab, format = "html", digits = 2))
```
:::
